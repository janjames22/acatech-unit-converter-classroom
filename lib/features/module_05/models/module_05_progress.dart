enum Module5CompletionStatus { notStarted, inProgress, complete }

final class Module5Progress {
  ...
  Module5CompletionStatus completionStatus({
    required int lessonCount,
    required int questionCount,
  }) {
    if (viewedLessonIds.length >= lessonCount &&
        practiceCompleted(questionCount)) {
      return Module5CompletionStatus.complete;
    }
    if (viewedLessonIds.isNotEmpty || practiceAttempts.isNotEmpty) {
      return Module5CompletionStatus.inProgress;
    }
    return Module5CompletionStatus.notStarted;
  }

  Module5Progress copyWith({
  ...
  factory Module5Progress.fromJson(Map<String, Object?> json) {
    if (json['schemaVersion'] != schemaVersion ||
        json['moduleId'] != 'module_05') {
      throw const FormatException('Unsupported Module 5 progress record.');
    }
    return Module5Progress(
  ...
  static Set<String> _stringSet(Object? value) {
    if (value is! List<Object?>) {
      throw const FormatException('Invalid Module 5 progress list.');
    }
    return value.map((item) {
      if (item is! String || item.isEmpty) {
        throw const FormatException('Invalid Module 5 progress identifier.');
      }
      return item;
    }).toSet();
  }
  ...
  static Map<String, int> _attemptMap(Object? value) {
    if (value is! Map<Object?, Object?>) {
      throw const FormatException('Invalid Module 5 attempt map.');
    }
    ...
    for (final entry in value.entries) {
      if (entry.key is! String ||
        ...
        throw const FormatException('Invalid Module 5 attempt entry.');
      }
